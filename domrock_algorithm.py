...
for ix in df_lojas['cidade'].drop_duplicates():
    print(ix)
    try:
        clusters = pd.read_excel('clusters.xlsx', sheet_name=ix)
    except:
        continue
    ...
    for iy in clusters.keys():
        print(iy)
        new_data = lojas_atendimento_clusters[(lojas_atendimento_clusters['cidade'] == ix) &
                                   (lojas_atendimento_clusters['cluster'] == iy)].drop_duplicates(subset=['id_loja_x'])
        ...
        tempo_limite = 100  # Tempo limite em segundos
